...
async def create_post(
    ...
    user_data=Depends(JWT().get_current_user),
):
    try:
        ...
        posts_handler = PostsHandler()
        return await posts_handler.create_post(
            file=file, post=post, user_id=user_data["user_id"]
        )
...
@posts_router.put(
    APIEndpoints.update_post + "/{post_id}", status_code=status.HTTP_202_ACCEPTED
)
def update_post(
    post_id: str, post: PostSchema, user_data=Depends(JWT().get_current_user)
):
    try:
        posts_handler = PostsHandler()
...
@posts_router.post(
    APIEndpoints.delete_post + "/{post_id}", status_code=status.HTTP_204_NO_CONTENT
)
