...
from scripts.db.mongo.posts.aggregations.post_details import PostDetailsAggregations
...
class PostsHandler:
    def __init__(self):
        ...
        self.publisher = Publisher(
            topic_path="projects/silver-approach-371713/topics/post-crud-events"
        )

    def get_all_posts(self, user_id: str):
        try:
            return self.posts.fetch_by_aggregate(
                pipelines=PostDetailsAggregations.get_all_post(user_id=user_id)
            )
        ...
        try:
            ...
            if file:
                ...
                file_url = await self.cloud_storage.upload_blob(
                    ...
                    destination_blob_name=post["post_id"]
                    + "."
