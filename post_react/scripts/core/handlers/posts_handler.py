...
class PostsHandler:
    def __init__(self):
        ...
        self.publisher = Publisher(
            topic_path='projects/silver-approach-371713/topics/post-react')
    ...
    def post_like_dislike(self, post_id: str, user_id: str):
        try:
            ...
            if user_id not in post['liked_by']:
                self.posts.like_post(
                    post_id=post_id, user_id=user_id)
                self.publisher.publish(data='post_liked', attributes={
                                       "post_id": post_id})
                like_count = len(post['liked_by']) + 1
            else:
                self.posts.disklike_post(
                    post_id=post_id, user_id=user_id)
                self.publisher.publish(data='post_disliked', attributes={
                                       "post_id": post_id})
                like_count = len(post['liked_by']) - 1
